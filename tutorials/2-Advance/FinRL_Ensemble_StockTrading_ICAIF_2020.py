...
print(f"数据集head: {df.head()}")
print(f"数据集tail: {df.tail()}")
print(f"数据集shape: {df.shape}")

print(f"按日期和股票排序: {df.sort_values(['date','tic']).head()}")
print(f"股票数量: {len(df.tic.unique())}")
print(f"统计: {df.tic.value_counts()}")
...
print(f"打印5条数据: {processed.sample(5)}")
